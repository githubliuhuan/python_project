...
# 以下注释部分从http://www.xicidaili.com/nn/获取一个髙匿ip用于伪装爬取（测试用）
...
# 爬取之前删除当月数据
def delete_gbif(c,m,year):
	if m[0] is '0':
		m = m[1:]
	else:
		pass
	sql = "DELETE FROM gbif WHERE year_1 = '"+year+"' AND month_1 = '"+m+"' AND scrapy_country = '"+c+"'"
	print(sql)
	try:
		cursor.execute(sql)
		conn.commit()
		print('%s 成功删除国家%s %s-%s月份数据！' % ((time.strftime("%Y-%m-%d %X", time.localtime())),c,year,m))
	except Exception as e:
		conn.rollback() #事务回滚
		print('事务处理失败 %s %s %s %s %s' % (time.strftime("%Y-%m-%d %X", time.localtime()),e,c,year,m))

# 解析入库
def insert_gbif(record,c,m,year):
...
if __name__ == '__main__':
	...
	cursor = conn.cursor()
	# 东亚六国
	country = ['KR','HK','TW','JP','RU','MO','MN','CN','KP']
	for c in country:
		# 定义list,放当月月份
		month_list = []
		...
		for m in month_list:
			num = 0
			while True:
				url = 'https://www.gbif.org/api/occurrence/search?advanced=false&country=%s&facet=country&facet=month&facetMultiselect=true&issue.facetLimit=1000&locale=en&month=%s&month.facetLimit=12&offset=%s&taxon_key=212&type_status.facetLimit=1000&year=%s,2017' % (c,m,str(num),year)
				print(url)
				html = get_html(url)
				...
				if len(record["results"]) is not 0:
					delete_gbif(c,m,year)
					flag = insert_gbif(record,c,m,year)
					print(flag)
					if flag is False:
						print('休眠3秒！')
						print('3..')
