...
def insert_ebird(url,c,S = None):
	if S is None:
		...
		# species_has_num = len(species_has)
		# species_obs_num = len(species_obs)
		for i in range(len(species_has)):
			...
			if html is not None:
				...
				# species_protocol = convert_format(checklist_o[0])
				# species_party_size = convert_format(checklist_o[1])
				# species_duration = convert_format(checklist_o[2])
				# species_distance = convert_format(checklist_o[3])
				# species_observers = convert_format(checklist_o[4])
				# species_comments = convert_format(checklist_o[5])
				# species_version = convert_format(checklist_o[6])
				# species_date_effort = "|".join([x for x in checklist[0].text.split('\n') if x != ''])
