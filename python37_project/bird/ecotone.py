...
def insert_ecotone(url,csv_name):
	...
	for id in range(1,line_num-1):
		...
		sql = 'insert into bird_ecotone (gps_pos_csv,idnr,gpsnumber,gpstime,smstime,latitude,longtitude,batteryvoltage,gpsdescription,temperature,gpsintervals,vhftelemetry,activity,gsmsignal) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)'
		try:
...
def delete_ecotone(csv_name):
	try:
		cursor.execute('DELETE FROM bird_ecotone WHERE gps_pos_csv = \'%s\'' % (csv_name))
		conn.commit()
	except Exception as e:
			conn.rollback() #事务回滚
			print('事务处理失败！%s %s'% (time.strftime("%Y-%m-%d %X", time.localtime())),e)
